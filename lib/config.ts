...
export const Links: Set<String> = new Set(navigationLinks.map(navigationLink => {
  if (navigationLink.pageId === null) {
    return navigationLink.url
  }
  return navigationLink.pageId
}));
...
export const redisUrl = getEnv(
  ...
  `rediss://default:${redisPassword}@${redisHost}:${redisPort}` // use https://console.upstash.com/ 
)
